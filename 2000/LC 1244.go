...
func Constructor() Leaderboard {
    ...
    pos := newTreap[int]()
    return Leaderboard{m, cnt, pos}
}


func (c *Leaderboard) AddScore(playerId int, score int)  {
    oldScore := c.m[playerId]
    c.m[playerId] += score
    if c.cnt[oldScore]--; c.cnt[oldScore] == 0 {
        c.pos.delete(oldScore)
    }
    newScore := c.m[playerId]
    if c.cnt[newScore]++; c.cnt[newScore] == 1 {
        c.pos.put(newScore)
    }
...
func (c *Leaderboard) Top(K int) (ans int) {
    for t := c.pos.max(); K > 0; t = c.pos.prev(t.key) {
        s := min(c.cnt[t.key], K)
        ans += t.key * s 
        K -= s
    }
...
func (c *Leaderboard) Reset(playerId int)  {
    x := c.m[playerId]
    if c.cnt[x]--; c.cnt[x] == 0 {
        c.pos.delete(x)
    }
    c.m[playerId] = 0
}
